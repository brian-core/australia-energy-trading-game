    ...
    <div className="space-y-3 font-[family-name:var(--f-mono)] text-[11px]">
      ...
      <div className="rounded-lg border p-2.5" style={{ borderColor: "var(--edge)" }}>
        <div className="grid grid-cols-3 gap-2">
          <div>
            <div className="text-sm">{money(state.cash)}</div>
            <div className="text-[9px] tracking-widest text-[var(--ink-soft)]">CASH</div>
          </div>
          <div>
            <div className="text-sm" style={{ color: incomePerH >= 0 ? "var(--gen)" : "#e2483d" }}>
              {money(incomePerH)}/h
            </div>
            <div className="text-[9px] tracking-widest text-[var(--ink-soft)]">INCOME NOW</div>
          </div>
          <div>
            <div className="text-sm">{money(companyValue(state))}</div>
            <div className="text-[9px] tracking-widest text-[var(--ink-soft)]">COMPANY VALUE</div>
          </div>
        </div>
        <button
          onClick={() => {
          ...
          className="mt-2 rounded border px-1.5 py-0.5 text-[9px] tracking-widest text-[var(--ink-soft)]"
      ...
      {state.fleet.length > 0 && (
        <div className="space-y-2">
          {state.fleet.map((asset) => {
            ...
            return (
              <div key={asset.id} className="rounded-lg border p-2.5" style={{ borderColor: "var(--edge)" }}>
                    ...
                    return (
                      <div key={task.id} className="flex items-center gap-2 text-[10px]">
                        ...
                        <button
                          ...
                          className="rounded border px-1.5 py-0.5 text-[9px] tracking-wider text-[var(--ink-soft)] disabled:opacity-30"
                          style={{ borderColor: "var(--edge)" }}
            ...
            return (
              <div key={f.name} className="flex items-center justify-between gap-2 text-[10px]">
                ...
                <button
                  onClick={() => buy(f)}
                  disabled={!affordable}
                  className="shrink-0 rounded border px-1.5 py-0.5 text-[9px] tracking-wider disabled:opacity-30"
      ...
      {/* Log */}
      <div className="rounded-lg border p-2.5" style={{ borderColor: "var(--edge)" }}>
        <div className="mb-1 text-[10px] tracking-widest text-[var(--ink-soft)]">SITE LOG</div>
        <div className="max-h-40 space-y-1 overflow-y-auto hud-scroll">
          {[...state.log].reverse().map((entry, i) => (
            <div key={`${entry.at}-${i}`} className="flex gap-1.5 text-[10px] leading-snug">
